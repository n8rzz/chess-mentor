...
from worker.weakness_package.aggregator import aggregate_by_pattern, build_cycles, classify_artifacts
...
def run_classification(conn: psycopg.Connection, user_id: str) -> dict[str, Any]:
    ...
    with conn.transaction():
        ...
        aggregations = aggregate_by_pattern(classified, games_analyzed=games_analyzed)
        cycles = build_cycles(
            aggregations,
        ...
        cycle_ids: list[str] = []
        events_by_theme = {aggregation.pattern: aggregation.events for aggregation in aggregations}

        for cycle in cycles:
            events = events_by_theme.get(cycle.pattern, [])
    ...
    summary = {
        "user_id": user_id,
        "games_analyzed": games_analyzed,
        "pattern_occurrences_created": len(classified),
        "pattern_cycles_created": len(cycle_ids),
        "cycle_ids": cycle_ids,
    }
