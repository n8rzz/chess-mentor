...
- [x] Idempotent imports and job retries
- [x] Partial failure visibility (batch-level + per-game)
- [x] Rails only reads DB status/artifacts from Python (design principle domain-models §23–24)

### Testing (PRD §16)

- [x] Rails: model/service/request specs for each workflow step
- [x] Python: unit + Stockfish integration + PGN→analysis→weakness→plan E2E
- [x] Capybara system spec: happy path (register → connect → import → wait → weaknesses → plan → complete assignment)
- [x] Create comprehensive test plan doc from milestone docs
  - [x] automate userflows as much as possible
...
7. [x] Track progress
8. [x] Demonstrate measurable reduction in targeted weakness frequency
